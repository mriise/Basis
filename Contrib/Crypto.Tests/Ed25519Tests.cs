...
namespace Basis.Contrib.Crypto
{
	public class Ed25519Tests
	{
		...
		private IList<Example> RfcTestVectors()
		{
			...
			{
				// Examples from https://datatracker.ietf.org/doc/html/rfc8032#section-7.1
				new(
		...
		public void SlightlyModifiedFromTestVectorsShouldFail()
		{
			...
			foreach (var e in testVectors)
			{
				...
				// Try mutating payload
				Payload badPayload;
				if (payload.V.Length == 0)
				{
					badPayload = new Payload(new byte[] { 0x69 });
				}
				else
				{
					var tmp = pubkey.V.ToArray(); // copy
